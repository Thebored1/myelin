...
export function createSourceSession(rawContext: object) {
	...
	async function handleSectionsReady(sections: PdfSection[]) {
	...
	async function openAttachedNote() {
		...
		if (!ctx.scratchpadSavedId) {
			...
			const workingNote = await invoke<NoteDocument>('load_note', {
				noteId: ctx.scratchpadSavedId
			});
		...
		ctx.pdfIngestionPromise = (async () => {
			try {
				const result = await invoke<{ status: 'cached' | 'indexed' | 'empty'; chunks: number }>(
					...
					{ docId: sourceId, source: sourceTitle, text, formatHint: 'pdfText' }
				);
				const entry = {
					time: Date.now(),
					kind: 'done',
				...
				if (ctx.debugInfo)
					ctx.debugInfo = { ...ctx.debugInfo, trace: [...ctx.debugInfo.trace, startEntry, entry] };
				...
				ctx.debugInfo = ctx.debugInfo
					? { ...ctx.debugInfo, trace: [...ctx.debugInfo.trace, startEntry, entry] }
					: {
	...
	async function openAttachPdfDialog() {
		...
		try {
			...
			// This set only supports a local filter and is not UI state.
		...
		setTimeout(
			() =>
				(
					ctx.attachPdfDialog?.querySelector('.link-search-input') as HTMLInputElement | null
				)?.focus(),
			50
		);
	}
	...
	function handlePdfSearchKeydown(e: KeyboardEvent) {
		if (e.key === 'ArrowDown') {
			e.preventDefault();
			ctx.pdfSelectedIndex = Math.min(ctx.filteredPdfs.length - 1, ctx.pdfSelectedIndex + 1);
		} else if (e.key === 'ArrowUp') {
			e.preventDefault();
			ctx.pdfSelectedIndex = Math.max(0, ctx.pdfSelectedIndex - 1);
		} else if (e.key === 'Enter') {
			e.preventDefault();
			if (ctx.filteredPdfs.length > 0) attachPdf(ctx.filteredPdfs[ctx.pdfSelectedIndex]);
		}
	}
