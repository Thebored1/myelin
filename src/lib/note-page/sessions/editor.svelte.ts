...
export function createEditorSession(ctx: Record<string, any>) {
			...
			const instance = new ctx.VditorConstructor(container, {
				...
				keydown: (e: KeyboardEvent) => {
					if ((e.ctrlKey || e.metaKey) && e.code === 'Comma') {
						e.preventDefault();
						const selector = e.shiftKey ? 'button[data-type="search-blocks"]' : 'button[data-type="link-note"]';
						(ctx.vditorContainer?.querySelector(selector) as HTMLButtonElement | null)?.click();
						return;
					}
					if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key.toLowerCase() === 'z') {
						e.preventDefault();
						(ctx.vditorContainer?.querySelector('button[data-type="redo"]') as HTMLButtonElement | null)?.click();
					}
				},
				input: (value: string) => {
					ctx.draftBody = value;
	...
	function scanForTransclusions() {
		...
		const links = ctx.vditorContainer.querySelectorAll('[data-type="a"]:not(.transclusion-wrapper)');
		links.forEach((linkWrapper: Element) => {
			...
			const urlMatch = (linkWrapper.textContent || '').match(/\]\(\/notes\/([^#]+)#([a-fA-F0-9]{6})\)$/);
			if (!urlMatch) return;
			...
			if (ctx.blockCache[cacheKey]) {
				...
				invoke<NoteDocument>('load_note', { noteId: targetNoteId }).then((loaded) => {
	...
	async function fetchRelatedNotes() {
		...
		try {
			...
			if (query) {
				...
				ctx.relatedNotes = res.results.map((r) => r.note).filter((n) => n.id !== ctx.note?.id).slice(0, 5);
			}
		} catch (e) {
			console.error(e);
		}
