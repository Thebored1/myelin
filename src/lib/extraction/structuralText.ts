export type StructuralTextItem = {
	str?: string;
	transform?: number[];
	height?: number;
	width?: number;
...
export function pdfItemsToStructuralText(items: StructuralTextItem[]): string {
	...
	const left = lines.filter(
		(line) => Math.max(...line.map((item) => item.x + item.width)) < mid - median
	);
	...
	const fullBefore = full.filter(
		(line) => line[0].y > Math.max(left[0]?.[0].y ?? -Infinity, right[0]?.[0].y ?? -Infinity)
	);
	const fullAfter = full.filter(
		(line) => line[0].y <= Math.max(left[0]?.[0].y ?? -Infinity, right[0]?.[0].y ?? -Infinity)
	);
		...
		.map((line) => {
			...
			const text = line
				.map((item, index) => {
					...
					const gapText =
						prior && item.x - (prior.x + prior.width) > Math.max(1, median * 0.12)
							? tableLike
								? ' | '
								: ' '
							: '';
...
export function htmlElementToStructuralText(root: Element): string {
	...
	clone
		.querySelectorAll('script, style, nav, footer, form, [hidden]')
