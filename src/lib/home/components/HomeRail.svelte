		...
		<div class="rail-list">
			...
			{:else}
				...
				<div class="ov-group new-actions">
					...
					<button class="ov-row ov-clickable ov-action" onclick={home.importFile} disabled={home.isBusy}>
						<span class="ov-key"
							><svg
								width="11"
								height="11"
				...
				<div class="ov-group">
					...
					<button class="ov-row ov-group-head" onclick={() => (home.notesExpanded = !home.notesExpanded)}>
						<span class="ov-key">
							<svg
								class="ov-chevron"
						...
						<span class="ov-val">{home.typeCounts.md + home.typeCounts.tex + home.typeCounts.ipynb}</span>
					</button>
					{#if home.notesExpanded}
						{#each home.NOTE_SUBTYPES as st}
							{#if home.typeCounts[st.type] > 0}
								<button
									class="ov-row ov-sub ov-clickable"
									class:active={home.activeTypeFilter === st.type}
									onclick={() => home.setTypeFilter(home.activeTypeFilter === st.type ? 'all' : st.type)}
								>
					...
					<button class="ov-row ov-group-head" onclick={() => (home.docsExpanded = !home.docsExpanded)}>
						<span class="ov-key">
							<svg
								class="ov-chevron"
				...
				<div class="ov-group">
					...
					<div class="ov-row">
						...
						<span class="ov-val"
							>{home.app?.indexState.lastIndexedAt ? home.agoLabel(home.app.indexState.lastIndexedAt) : '—'}</span
						>
					</div>
					<div class="ov-row">
					...
					<div class="ov-row">
						...
						<span
							...
							title={home.provider?.resolved?.modelPath ?? home.provider?.config?.modelPath ?? 'No model selected'}
							>{home.modelFileName(home.provider?.resolved?.modelPath ?? home.provider?.config?.modelPath)}</span
						>
					</div>
					<div class="ov-row">
