<script lang="ts">
	...
	async function importFile(event: Event) {
		...
		try {
			...
			const imported = await themeController.importTheme(
				JSON.parse(await file.text()) as ColorTheme
			);
	...
	const previewTokens = $derived(
		new Map($availableThemes.map((theme) => [theme.id, compileTheme(theme)]))
	);
</script>
...
<section class="settings-section theme-settings">
	...
	<p class="description">
		...
		and shared with the Quick Capture window.
	</p>
		...
		<div class="accent-hero">
			<div class="accent-hero-heading">
				...
				<p class="compute-hint">
					Pick one color and the entire app recolors: buttons, links, selection, hovers, focus
					states, and a subtle surface wash. Applied to {$activeColorTheme?.name ??
						'the active theme'}.
				</p>
			</div>
			<AccentPicker
				value={accentDraft}
				mode={$activeColorTheme?.mode ?? 'dark'}
				onchange={(hex) => (accentDraft = hex)}
			/>
			<div class="theme-actions">
				<button class="browse-btn primary" onclick={() => void applyAccentDraft()} disabled={busy}
					>Apply accent</button
				>
				<span class="accent-hint"
					>Creates an editable copy that you can fine-tune and save below.</span
				>
			</div>
		</div>
	{/if}
	...
	<div class="theme-gallery">
		{#each $availableThemes as available (available.id)}
			...
			>
				<span
					class="theme-swatch"
					style={`background:${tokens?.['bg-page']}; color:${tokens?.['text-primary']}; border-color:${tokens?.['accent-100']}`}
				>
	...
	<div class="theme-actions">
		<button class="browse-btn" onclick={() => void duplicateCurrent()} disabled={busy}
			>Create editable copy</button
		>
		<button class="browse-btn" onclick={() => importInput?.click()} disabled={busy}
			>Import theme</button
		>
		...
		<input
			...
			type="file"
			accept="application/json,.myelin-theme.json"
		...
		<div class="theme-editor" aria-label="Custom theme editor">
			<div class="theme-editor-heading">
				<div>
					...
					<p class="compute-hint">
						Changes preview immediately and are not saved until you click Save theme.
					</p>
				</div>
				<input
			...
			<div class="theme-color-grid">
				{#each paletteFields as [key, label] (key)}
			...
			<div class="theme-contrast-list">
				{#each draftContrast as check, checkIndex (`${check.foreground}-${check.background}-${checkIndex}`)}
			...
			<div class="theme-actions">
				...
				<button class="browse-btn primary" onclick={() => void saveDraft()} disabled={busy}
					>Save theme</button
				>
			</div>
		</div>
...
<style>
	...
	.theme-contrast-list .contrast-fail {
		color: var(--danger-text);
	}
