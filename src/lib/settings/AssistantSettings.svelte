...
<section class="settings-section">
	...
	<p class="description">
		Two independent assists for how the model uses tools. Hover each <span class="info-dot">i</span> for
		details and caveats.
	</p>
		...
		<span class="toggle-text">
			<span class="toggle-label">
				...
				<span class="info" role="note" aria-label="About per-message tool gating">
					...
					<span class="info-pop"
						>Offers the model only the tools its message seems to need, chosen by keyword heuristics
						— brittle and not model-agnostic. It can <strong
		...
		<span class="toggle-text">
			<span class="toggle-label">
				...
				<span class="info" role="note" aria-label="About deterministic format and find">
					<span class="info-dot">i</span>
					<span class="info-pop"
						>In-code correctness assists that don’t withhold tools — they make the result reliable:
						formatting (strip headings/bold/bullets, change case, convert lists) is applied by exact
						rules instead of the model rewriting the whole note; exact-word lookups use a reliable
						search; and a guard prevents accidentally wiping a note during an edit. <strong
							>On by default.</strong
						> (Surgical deletes — remove a paragraph/heading/section — always apply, regardless of this
						toggle.)</span
					>
				</span>
			</span>
...
<section class="settings-section">
	...
	<p class="description">
		...
		calls back into Myelin for the real note / search / web tools. These settings tune that sidecar.
		The sidecar binary is required for AI agent and tool-calling features. Run <code
	...
	<div class="input-group full-width">
		...
		<p class="compute-hint">
			...
			policy needs them. Native is usually best for larger models. Prompt tools can help smaller or
			unreliable models, but may reduce quality on larger models.
		</p>
	</div>
	...
	<label class="toggle-row">
		...
		<span class="toggle-text">
			...
			<span class="toggle-hint"
				>Restricts text-form tool calls to valid structured syntax. More reliable, but less flexible
				and slower.</span
			>
		</span>
	</label>
	...
	<label class="toggle-row">
		...
		<span class="toggle-text">
			...
			<span class="toggle-hint"
				>Prevents prose when a request is classified as an operation. Useful for weak models; can be
				too restrictive for larger models.</span
			>
		</span>
	</label>
	...
	<p class="compute-hint">
		Explicit path to the sidecar binary. Blank = bundled/resource-dir lookup. If it is missing, run <code
			>npm run build:sidecar</code
		>
