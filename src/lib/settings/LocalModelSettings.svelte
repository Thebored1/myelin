...
<section class="settings-section">
	...
	<p class="description">
		Select a model to use for local AI features. It runs completely offline on your device and is
		saved in app settings, not inside the notes workspace. <strong
	...
	<div class="compute-device">
		...
		<p class="compute-hint">
			...
			{:else if settings.backendPreference === 'vulkan'}
				Power-saving: runs on the integrated GPU via Vulkan. The app still manages offload and falls
			...
			{:else if settings.backendPreference === 'metal'}
				macOS GPU acceleration: runs through Apple Metal. Requires the downloaded Metal backend and
	...
	<h2>Web Search</h2>
	<p class="description">
		Privacy-first web search for the assistant. Set a SearXNG instance URL, or leave blank to use
		the no-key DuckDuckGo fallback.
	</p>
	<div class="model-picker">
		<input
			type="text"
	...
	<p class="description">
		A validated embedding GGUF powers semantic search over your notes and lets the assistant search
		ingested documents (PDFs, books). Known models receive their required query/document format;
		other compatible models use plain text. It runs as a second local server.
	</p>
	<div class="model-picker">
	...
	<div class="model-picker">
		...
		>
		{#if settings.rerankerModelPath}<button class="browse-btn" onclick={settings.clearRerankerModel}
				>Clear</button
			>{/if}
	</div>
	...
	<h2>PDF OCR</h2>
	<p class="description">
		OCR is reserved for a future bundled runtime. It is currently dormant and native PDF extraction
		remains active.
	</p>
	...
	<div class="input-group full-width" style="margin-top: 1rem;">
		<div>
			...
			<div style="font-size: 0.8em; color: var(--text-error); margin-top: 4px;">
				<strong>CRITICAL NOTE:</strong> Because of how system processes work, you cannot put spaces
				in a single box! If you wanted to add <code>--threads 8</code>, you must put
				<code>--threads</code>
