...
micromamba.exe --help
IF %ERRORLEVEL% NEQ 0 exit /B 1
set "PATH=%CONDA_PREFIX%\Library\usr\bin;%CONDA_PREFIX%\Library\mingw-w64\bin;%PATH%" && bash mamba/dev/check_micromamba_windows_dlls.sh "%LIBRARY_BIN%\micromamba.exe"
IF %ERRORLEVEL% NEQ 0 exit /B 1
mkdir %TEMP%\mamba
IF %ERRORLEVEL% NEQ 0 exit /B 1
