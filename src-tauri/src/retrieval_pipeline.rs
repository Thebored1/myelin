...
pub fn blend_reranker_scores(chunks: &mut [RetrievedChunk], scores: &[f32]) {
    if chunks.is_empty() || chunks.len() != scores.len() { return; }
    let min = scores.iter().copied().fold(f32::INFINITY, f32::min);
    let max = scores.iter().copied().fold(f32::NEG_INFINITY, f32::max);
    let span = (max - min).max(f32::EPSILON);
    let count = chunks.len().max(1) as f32;
    let mut ranked = chunks.iter_mut().zip(scores.iter().copied()).collect::<Vec<_>>();
...
const STOP: &[&str] = &[
    "a", "an", "and", "are", "as", "at", "be", "does", "find", "for", "from", "in", "is",
    "it", "me", "of", "on", "or", "show", "tell", "that", "the", "this", "to", "what", "which",
    "with", "about", "document", "paper", "pdf",
];
...
impl RetrievalQuery {
    pub fn analyze(input: &str) -> Self {
        ...
        for raw in raw_terms {
            ...
            if seen.insert(lower.clone()) {
                let weight = if is_identifier || raw.chars().any(|c| c.is_ascii_digit()) { 1.5 } else { 1.0 };
                weighted_terms.push(WeightedTerm { term: lower, weight });
            }
        }
        for url in &urls {
            for part in split_terms(url) {
                ...
                if !lower.is_empty() && seen.insert(lower.clone()) {
                    weighted_terms.push(WeightedTerm { term: lower, weight: 1.5 });
                }
            }
        }
        ...
        let intent = if ["compare", "difference", "versus", " vs "].iter().any(|m| lower.contains(m)) {
            RetrievalIntent::Comparison
        } else if ["quote", "recite", "transcribe", "verbatim"].iter().any(|m| lower.contains(m)) {
            RetrievalIntent::Quotation
        } else if ["summarize", "summary", "overview", "all ", "every ", "across"].iter().any(|m| lower.contains(m)) {
            RetrievalIntent::BroadSummary
        } else if ["why", "how does", "connect", "relate"].iter().any(|m| lower.contains(m)) {
            RetrievalIntent::Synthesis
        } else if ["then", "after that", "also"].iter().any(|m| lower.contains(m)) {
            RetrievalIntent::MultiHop
        } else {
            RetrievalIntent::Lookup
        };
        Self {
            original: authoritative.to_string(), normalized, phrases, weighted_terms, entities, identifiers, urls,
            intent, requested_sources: Vec::new(),
        }
    }
    ...
    pub fn analyze_cached(input: &str) -> Self {
        ...
        if cache.len() > CAPACITY { cache.pop_front(); }
        value
    }
    ...
    pub fn fts_query(&self) -> String {
        self.weighted_terms.iter().map(|term| term.term.as_str()).collect::<Vec<_>>().join(" ")
    }
}

pub fn latest_user_question(input: &str) -> &str {
    let input = input.strip_prefix("Latest question:").unwrap_or(input);
    input.find("\nPrevious ").map(|end| &input[..end]).unwrap_or(input).trim()
}

pub fn lexical_text(source: &str, section: Option<&str>, text: &str) -> String {
    let mut pieces = vec![source.to_string()];
    if let Some(section) = section.filter(|value| !value.trim().is_empty()) { pieces.push(section.to_string()); }
    pieces.push(text.to_string());
    let combined = pieces.join("\n");
    let mut additions = Vec::new();
    for token in combined.split_whitespace() {
        if token.contains('_') || token.contains('/') || token.contains('\\') || token.contains("::") || has_camel_case(token) {
            additions.extend(split_identifier(token));
        }
    }
    additions.extend(cjk_bigrams(&combined));
    format!("{}\n{}", combined.split_whitespace().collect::<Vec<_>>().join(" "), additions.join(" "))
}
...
pub fn rank_candidates(
    ...
    for (rank, chunk) in vector_hits.into_iter().enumerate() {
        ...
        candidates.entry(key).or_insert_with(|| RetrievalCandidate {
            chunk, vector_rank: Some(rank + 1), fts_rank: None, cosine_similarity: Some(similarity), bm25_score: None,
            bm25_normalized: None, term_coverage: 0.0, phrase_match: 0.0, field_match: 0.0,
            base_score: 0.0, reranker_score: None, final_score: 0.0,
        });
    }
    for (rank, chunk) in fts_hits.into_iter().enumerate() {
        let key = (chunk.doc_id.clone(), chunk.chunk_index);
        let score = chunk.bm25_score.filter(|value| value.is_finite() && *value >= 0.0);
        if let Some(existing) = candidates.get_mut(&key) {
            existing.fts_rank = Some(rank + 1);
            existing.bm25_score = score;
        } else {
            candidates.insert(key, RetrievalCandidate {
                chunk, vector_rank: None, fts_rank: Some(rank + 1), cosine_similarity: None, bm25_score: score,
                bm25_normalized: None, term_coverage: 0.0, phrase_match: 0.0, field_match: 0.0,
                base_score: 0.0, reranker_score: None, final_score: 0.0,
            });
        }
    }
    let best_bm25 = candidates.values().filter_map(|candidate| candidate.bm25_score).fold(0.0_f32, f32::max);
    let has_vector = candidates.values().any(|candidate| candidate.vector_rank.is_some());
    let has_fts = candidates.values().any(|candidate| candidate.fts_rank.is_some());
    let mut values = candidates.into_values().collect::<Vec<_>>();
    for candidate in &mut values {
        candidate.bm25_normalized = candidate.bm25_score.map(|score| if best_bm25 > 0.0 { (score / best_bm25).clamp(0.0, 1.0) } else { 0.0 });
        let searchable = format!("{}\n{}\n{}", candidate.chunk.source, candidate.chunk.section_start.as_deref().unwrap_or_default(), candidate.chunk.text).to_lowercase();
        ...
        let (semantic_weight, bm25_weight, coverage_weight, field_weight) = match (has_vector, has_fts) {
    ...
    values.sort_by(|left, right| {
        right.final_score.total_cmp(&left.final_score)
            .then_with(|| right.base_score.total_cmp(&left.base_score))
            .then_with(|| left.vector_rank.or(left.fts_rank).unwrap_or(usize::MAX).cmp(&right.vector_rank.or(right.fts_rank).unwrap_or(usize::MAX)))
...
pub fn select_diverse(
    ...
    let per_document_cap = if scoped_to_one_document { usize::MAX } else if matches!(intent, RetrievalIntent::BroadSummary | RetrievalIntent::Comparison | RetrievalIntent::Synthesis) { 4 } else { 2 };
    ...
    while !pending.is_empty() && selected.len() < FINAL_SEED_COUNT {
        ...
        for (index, candidate) in pending.iter().enumerate() {
            if per_document.get(&candidate.chunk.doc_id).copied().unwrap_or(0) >= per_document_cap { continue; }
            if selected.iter().any(|other: &RetrievalCandidate| source_overlap(&candidate.chunk, &other.chunk) >= 0.60) { continue; }
            let redundancy = selected.iter().map(|other: &RetrievalCandidate| {
                source_overlap(&candidate.chunk, &other.chunk).max(shingle_jaccard(&candidate.chunk.text, &other.chunk.text))
            }).fold(0.0_f32, f32::max);
            let score = MMR_LAMBDA * candidate.final_score - (1.0 - MMR_LAMBDA) * redundancy;
            if best.as_ref().is_none_or(|(_, current): &(usize, f32)| score > *current) { best = Some((index, score)); }
        }
        let Some((index, _)) = best else { break; };
        let candidate = pending.remove(index);
        *per_document.entry(candidate.chunk.doc_id.clone()).or_default() += 1;
...
pub fn source_overlap(left: &RetrievedChunk, right: &RetrievedChunk) -> f32 {
    if left.doc_id != right.doc_id { return 0.0; }
    let (Some(start), Some(end), Some(other_start), Some(other_end)) = (left.char_start, left.char_end, right.char_start, right.char_end) else { return 0.0; };
...
fn shingle_jaccard(left: &str, right: &str) -> f32 {
    let shingles = |value: &str| -> HashSet<String> {
        ...
        terms.windows(2).map(|pair| format!("{}\u{0}{}", pair[0], pair[1])).collect()
    };
...
fn term_coverage(query: &RetrievalQuery, text: &str) -> f32 {
    let total = query.weighted_terms.iter().map(|term| term.weight).sum::<f32>();
    if total <= 0.0 { return 0.0; }
    query.weighted_terms.iter().filter(|term| contains_term(text, &term.term)).map(|term| term.weight).sum::<f32>() / total
}

fn phrase_match(query: &RetrievalQuery, text: &str) -> f32 {
    if query.phrases.is_empty() { return 0.0; }
    let hits = query.phrases.iter().filter(|phrase| text.contains(&phrase.to_lowercase())).count();
...
fn field_match(query: &RetrievalQuery, chunk: &RetrievedChunk) -> f32 {
    ...
    let section = chunk.section_start.as_deref().unwrap_or_default().to_lowercase();
    let mut score: f32 = 0.0;
    for term in &query.weighted_terms {
        if contains_term(&source, &term.term) { score += 0.30 * term.weight.min(1.5); }
        if contains_term(&section, &term.term) { score += 0.35 * term.weight.min(1.5); }
    }
    for phrase in &query.phrases {
        if source.contains(&phrase.to_lowercase()) || section.contains(&phrase.to_lowercase()) { score += 0.35; }
    }
    score.min(1.0)
}
    ...
    for (index, ch) in input.char_indices() {
        if matches!(ch, '\"' | '“' | '”' | '\'') {
            if quote.take().is_some() {
                ...
                if !phrase.is_empty() { phrases.push(phrase.to_string()); }
            } else {
                quote = Some(ch);
...
fn split_terms(input: &str) -> Vec<String> {
    input.split(|c: char| !(c.is_alphanumeric() || matches!(c, '_' | '-' | '/' | '\\' | ':' | '.')))
        .filter(|part| !part.is_empty())
        .flat_map(|part| {
...
fn split_identifier(value: &str) -> Vec<String> {
    value.split(|c: char| matches!(c, '_' | '-' | '/' | '\\' | ':' | '.'))
...
fn has_camel_case(value: &str) -> bool { value.chars().any(char::is_lowercase) && value.chars().any(char::is_uppercase) }

fn cjk_bigrams(value: &str) -> Vec<String> {
    let chars = value.chars().collect::<Vec<_>>();
    chars.windows(2).filter_map(|pair| (is_cjk(pair[0]) && is_cjk(pair[1])).then(|| pair.iter().collect())).collect()
}

fn is_cjk(ch: char) -> bool { matches!(ch as u32, 0x3400..=0x4DBF | 0x4E00..=0x9FFF | 0xF900..=0xFAFF | 0x3040..=0x30FF | 0xAC00..=0xD7AF) }

fn contains_term(text: &str, term: &str) -> bool {
    if term.chars().count() <= 2 || term.chars().any(is_cjk) { text.contains(term) }
    else { text.split(|c: char| !c.is_alphanumeric()).any(|word| word == term) || text.contains(term) }
}
...
mod tests {
    ...
    fn query_analysis_preserves_identifiers_and_phrases() {
        let query = RetrievalQuery::analyze("Find `foo_bar` at https://example.test/a-b and \"exact phrase\"");
    ...
    fn mmr_drops_overlapping_seeds() {
        let candidate = |index, start, end| RetrievalCandidate {
            chunk: RetrievedChunk { doc_id: "d".into(), chunk_index: index, char_start: Some(start), char_end: Some(end), text: format!("chunk {index}"), ..Default::default() },
            vector_rank: Some(index as usize), fts_rank: None, cosine_similarity: Some(1.0), bm25_score: None,
            bm25_normalized: None, term_coverage: 1.0, phrase_match: 0.0, field_match: 0.0,
            base_score: 1.0, reranker_score: None, final_score: 1.0 - index as f32 / 10.0,
        };
        let selected = select_diverse(vec![candidate(0, 0, 100), candidate(1, 20, 100), candidate(2, 120, 200)], RetrievalIntent::Lookup, false);
