...
pub fn applied_path(app_data: &Path) -> PathBuf {
    app_data.join(APPLIED_FILE_NAME)
}
...
pub fn validate(config: &AiConfigFile) -> Vec<AiConfigError> {
    ...
    if config.version != CONFIG_VERSION {
        errors.push(err(
            "/version",
            "schema",
            format!("unsupported version {}", config.version),
        ));
    }
    ...
    for (id, profile) in &config.profiles {
        ...
        if !profile.model_path.is_absolute() {
            errors.push(err(
                &format!("/profiles/{id}/modelPath"),
                "semantic",
                "model path must be absolute",
            ));
        }
        if !(1024..=65535).contains(&profile.server.port) {
            errors.push(err(
                &format!("/profiles/{id}/server/port"),
                "semantic",
                "port must be between 1024 and 65535",
            ));
        }
        if profile.inference.context_size == 0 {
            errors.push(err(
                &format!("/profiles/{id}/inference/contextSize"),
                "semantic",
                "context size must be positive",
            ));
        }
        if !profile.inference.temperature.is_finite() || profile.inference.temperature < 0.0 {
            errors.push(err(
                &format!("/profiles/{id}/inference/temperature"),
                "semantic",
                "temperature must be finite and non-negative",
            ));
        }
        if !profile.inference.top_p.is_finite() || !(0.0..=1.0).contains(&profile.inference.top_p) {
            errors.push(err(
                &format!("/profiles/{id}/inference/topP"),
                "semantic",
                "topP must be between 0 and 1",
            ));
        }
        if !config.runtimes.contains_key(&profile.runtime)
            && !matches!(profile.runtime.as_str(), "stock" | "bee")
        {
            errors.push(err(
                &format!("/profiles/{id}/runtime"),
                "semantic",
    ...
    for (id, runtime) in &config.runtimes {
        validate_id(id, "/runtimes", &mut errors);
        validate_runtime(id, runtime, &mut errors);
    }
    ...
    validate_retrieval_model(
        "/embedding",
        config.embedding.enabled,
        config.embedding.model_path.as_ref(),
        config.embedding.context_size,
        config.embedding.parallelism,
        &mut errors,
    );
    validate_retrieval_model(
        "/reranker",
        config.reranker.enabled,
        config.reranker.model_path.as_ref(),
        config.reranker.context_size,
        config.reranker.parallelism,
        &mut errors,
    );
    if config.embedding.max_inputs_per_request == 0 || config.embedding.batch_token_limit == 0 {
        errors.push(err(
            "/embedding",
            "semantic",
            "embedding request limits must be positive",
        ));
    }
    if config.reranker.candidate_cap == 0
        || config.reranker.candidate_cap > 12
        || config.reranker.output_cap == 0
        || config.reranker.output_cap > config.reranker.candidate_cap
    {
        errors.push(err(
            "/reranker",
            "semantic",
            "reranker caps must be 1..12 and output cannot exceed candidates",
        ));
    }
    if config.reranker.timeout_ms == 0
        || config.reranker.timeout_ms > 5_000
        || !(0.0..=1.0).contains(&config.reranker.blend_weight)
    {
        errors.push(err(
            "/reranker",
            "semantic",
            "reranker timeout or blend weight is invalid",
        ));
    }
    errors
}

fn validate_retrieval_model(
    root: &str,
    enabled: bool,
    path: Option<&PathBuf>,
    context: Option<u32>,
    parallelism: u32,
    errors: &mut Vec<AiConfigError>,
) {
    if enabled && path.is_none() {
        errors.push(err(root, "semantic", "enabled model requires modelPath"));
    }
    if path.is_some_and(|path| !path.is_absolute()) {
        errors.push(err(
            &format!("{root}/modelPath"),
            "semantic",
            "model path must be absolute",
        ));
    }
    if context == Some(0) || parallelism == 0 {
        errors.push(err(
            root,
            "semantic",
            "context size and parallelism must be positive",
        ));
    }
}
...
fn validate_runtime(id: &str, runtime: &RuntimeDefinition, errors: &mut Vec<AiConfigError>) {
    if !(1..=4096).contains(&runtime.cache_reuse_tokens) {
        errors.push(err(
            &format!("/runtimes/{id}/cacheReuseTokens"),
            "semantic",
            "must be between 1 and 4096",
        ));
    }
    for key in runtime.environment.keys() {
        if matches!(
            key.as_str(),
            "PATH" | "HOME" | "LD_LIBRARY_PATH" | "DYLD_LIBRARY_PATH"
        ) {
            errors.push(err(
                &format!("/runtimes/{id}/environment/{key}"),
                "semantic",
                "environment key is managed by Myelin",
            ));
        }
    }
    if let RuntimeSource::Download {
        url,
        sha256,
        binary_path,
        ..
    } = &runtime.source
    {
        ...
        if sha256.len() != 64 || !sha256.chars().all(|c| c.is_ascii_hexdigit()) {
            errors.push(err(
                &format!("/runtimes/{id}/source/sha256"),
                "semantic",
                "SHA-256 must be 64 hexadecimal characters",
            ));
        }
        if binary_path.as_ref().is_some_and(|p| {
            p.is_absolute()
                || p.components()
                    .any(|c| matches!(c, std::path::Component::ParentDir))
        }) {
            errors.push(err(
                &format!("/runtimes/{id}/source/binaryPath"),
                "semantic",
                "binary path must remain inside the archive",
            ));
        }
    }
}

fn err(path: &str, category: &str, message: impl Into<String>) -> AiConfigError {
    AiConfigError {
        path: path.into(),
...
pub fn load(app_data: &Path) -> Result<AiConfigFile> {
    ...
    serde_json::from_str(&text)
        .with_context(|| format!("invalid AI config JSON at {}", path.display()))
}
...
fn migrate_legacy(app_data: &Path) -> Result<AiConfigFile> {
    ...
    let engine = get("inferenceEngine")
        .and_then(|v| v.as_str().map(str::to_owned))
        .unwrap_or_else(|| "llama_cpp".into());
    let runtime_id = if engine == "beellama" { "bee" } else { "stock" };
    let model = get("modelPath")
        .and_then(|v| v.as_str().map(PathBuf::from))
        .unwrap_or_else(|| app_data.join("model.gguf"));
    let mut config = default_config();
    config.runtimes.insert(
        "stock".into(),
        RuntimeDefinition {
            display_name: "Stock llama.cpp".into(),
            source: RuntimeSource::Bundled {
                runtime: BuiltinRuntime::Stock,
            },
    ...
    config.runtimes.insert(
        "bee".into(),
        RuntimeDefinition {
            display_name: "BeeLlama".into(),
            source: RuntimeSource::Bundled {
                runtime: BuiltinRuntime::Bee,
            },
    ...
    if let Some(v) = get("contextSize").and_then(|v| v.as_u64()) {
        inference.context_size = v as u32;
    }
    if let Some(v) = get("gpuLayers").and_then(|v| v.as_i64()) {
        inference.gpu_layers = Some(v as i32);
    }
    if let Some(v) = get("threads").and_then(|v| v.as_u64()) {
        inference.threads = Some(v as u32);
    }
    if let Some(v) = get("temperature").and_then(|v| v.as_f64()) {
        inference.temperature = v as f32;
    }
    if let Some(v) = get("topP").and_then(|v| v.as_f64()) {
        inference.top_p = v as f32;
    }
    if let Some(v) = get("thinking").and_then(|v| v.as_bool()) {
        inference.thinking = v;
    }
    if let Some(v) = get("autoOffload").and_then(|v| v.as_bool()) {
        inference.auto_offload = v;
    }
    if let Some(v) = get("maxTurns").and_then(|v| v.as_u64()) {
        inference.max_turns = v as u32;
    }
    if let Some(v) = get("extraArgs").and_then(|v| v.as_array().cloned()) {
        inference.extra_args = v
            .into_iter()
    ...
    let server = ServerConfig {
        ...
        port: get("port").and_then(|v| v.as_u64()).unwrap_or(39281) as u16,
    };
...
pub fn status(app_data: &Path) -> AiConfigStatus {
    ...
    let errors = config
        .as_ref()
        .map(validate)
        .unwrap_or_else(|| vec![err("/", "syntax", "unable to parse ai-config.json")]);
    let candidate_hash = config.as_ref().and_then(|c| canonical_hash(c).ok());
    let applied = fs::read_to_string(applied_path(app_data))
        .ok()
    ...
    AiConfigStatus {
        ...
        candidate_hash,
        applied_hash,
        validation_state: if config.is_none() {
            "invalid".into()
        ...
        ai_available: errors.is_empty() && profile.is_some(),
        errors,
    }
}
