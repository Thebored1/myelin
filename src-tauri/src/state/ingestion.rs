...
impl AppState {
    ...
    pub(crate) async fn clone_document_ingestion(&self, source_id: &str, target_id: &str) -> Result<bool> {
        let entry = {
            let _guard = self.inner.note_ingest_manifest_lock.lock().await;
            self.load_note_ingestion_manifest().entries.get(source_id).cloned()
        };
        let Some(entry) = entry else { return Ok(false); };
        if !crate::rag_copy::clone_document(&self.rag_dir(), source_id, target_id).await? { return Ok(false); }
        let _guard = self.inner.note_ingest_manifest_lock.lock().await;
