...
impl Tool for WriteNoteTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        if self.turn.policy.targeted_write
            ...
            && !wants_targeted_deletion(&self.turn.question)
        {
            return Err(ToolError {
        ...
        let armed_selection = self.turn.selection.clone();
        ...
        if let Err(msg) =
            check_tool_approval(&self.turn, display_name, &existing.title, &content).await
        {
            return Ok(msg);
        }
...
impl Tool for AppendNoteTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let content = clean_note_content(&normalize_append_content(
            &existing.body,
            &strip_prompt_markers(&args.content),
        ));
...
impl Tool for PrependNoteTool {
    ...
    async fn definition(&self, _prompt: String) -> ToolDefinition {
        let (_, description, params) =
            tool_contract("prepend_note").expect("prepend_note contract");
        ToolDefinition {
