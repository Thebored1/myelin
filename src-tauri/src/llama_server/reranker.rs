...
pub async fn start_reranker_server(
    ...
    command
        ...
        .arg("--reranking")
        .arg("--ctx-size")
        .arg(context_tokens.clamp(128, 8192).to_string())
        .arg("--batch-size")
        .arg("2048")
        .arg("--ubatch-size")
        .arg(context_tokens.min(2048).to_string())
        .arg("--parallel")
        .arg("1")
        .arg("--no-warmup")
        .stdout(Stdio::null())
    ...
    let mut child = command
        .spawn()
        .with_context(|| format!("failed to spawn reranker server: {}", executable.display()))?;
    ...
    for _ in 0..80 {
        if client
            ...
            .unwrap_or(false)
        {
            return Ok(ManagedRerankerServer {
                child,
        ...
        if let Ok(Some(status)) = child.try_wait() {
            bail!(
                "reranker server exited early ({status}) while loading {}",
                model_path.display()
            );
        }
    ...
    let _ = child.kill();
    bail!(
        "reranker server did not become healthy while loading {}",
        model_path.display()
    )
}

pub async fn stop_reranker_server(server: &mut ManagedRerankerServer) {
...
pub fn resolve_reranker_executable(app_data_dir: &Path) -> Result<PathBuf> {
    resolve_embedding_executable(app_data_dir)
}
